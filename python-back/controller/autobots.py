		...
		while True:
				try:				
						...
						if ((float(currentCapital)> 5000) and (target_buy == True)):
								return upbit.buy_market_order(marketCode, currentCapital) # 잔고 전부 매수 #
						if ((float(capital_value))>5000 and (target_sell == True)):		
								return upbit.sell_market_order(marketCode, owned_balance) # 전부 매도 #

						## 보유 자산 가치가 10000원 이하일 때 break
						if ((float(currentCapital) < 5000) and (float(capital_value) < 5000)): 
								break
						## 초기 자본금 대비 12% 누적 수익이 났을 때 break
						if ((float(currentCapital) + float(capital_value)) > float(start_capital) * 1.12):
								break
