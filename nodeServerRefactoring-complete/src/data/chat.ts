...
export async function getNick(nickname?: string) {
  return Chat.findOne({ where: { nickname } }).then((data) => {
    return data;
  });
}
export async function findMessage(nickname: string, repoTime: string) {
  return Chat.findOne({ where: { nickname, repoTime } });
}
// 유저별 보낸 메시지 가져오기
export async function allNickM(nickname?: string) {
  return Chat.findAll({ where: { nickname } }).then((data) => {
    return data;
  });
}

export async function sendText(newM: any) {
  return Chat.create(newM);
}

export async function delText(nickname: string, repoTime: string) {
