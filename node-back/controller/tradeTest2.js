...
function postRequest(url, headers, data) {
  ...
  resp = callPost(url, (headers = headers), (data = data));
  data = resp.JSON(); // 나중에 확인
  return data;
}
...
class Upbit {
  ...
  requestHeaders(query = null) {
    ...
    if (!query) {
      const m = crypto.createHash('sha512').update(urlencode(query)).encode();
  ...
  marketOrder(market, side, price) {
    ...
    const data = { market, side, price, ord_type: 'price' };
    const headers = this.requestHeaders(data);
